package com.timecat.show.window;
...
public abstract class StandOutWindow extends Service {
    ...
    public static final String APPS_NOTIFICATION_CHANNEL_ID = "com.time.cat.channel.APPsNotification";
    public static final String APPS_HIDDEN_NOTIFICATION_CHANNEL_ID = "com.time.cat.channel.APPsHiddenNotification";
    ...
    /**
     * StandOut window id: You may use this id when you want it to be disregarded. The system makes no
     * distinction for this id; it is only used to improve code readability.
     */
...
//            startForeground(startId, lastNotification);
//        }TODO fix startForegroundService
        ...
        if (intent != null) {
            ...
            if (WindowAgreement.ACTION_SHOW.equals(action) || WindowAgreement.ACTION_RESTORE.equals(action)) {
                show(id);
            } else if (WindowAgreement.ACTION_HIDE.equals(action)) {
                hide(id);
            } else if (WindowAgreement.ACTION_CLOSE.equals(action)) {
                close(id);
            } else if (WindowAgreement.ACTION_CLOSE_ALL.equals(action)) {
                closeAll();
            } else if (WindowAgreement.ACTION_SEND_DATA.equals(action)) {
    ...
    /**
     * Return the name of every window in this implementation. The name will appear in the default
     * implementations of the system window decoration title and notification titles.
     *
     * @return The name.
     */
    ...
    /**
     * Return the icon resource for every window in this implementation. The icon will appear in the
     * default implementations of the system window decoration and notifications.
     *
     * @return The icon.
     */
    ...
    /**
     * Create a new {@link View} corresponding to the id, and add it as a child to the frame. The view
     * will become the contents of this StandOut window. The view MUST be newly created, and you MUST
     * attach it to the frame.
     * <p>
     * <p>
     * If you are inflating your view from XML, make sure you use {@link LayoutInflater#inflate(int,
     * ViewGroup, boolean)} to attach your view to frame. Set the ViewGroup to be frame, and the
     * boolean to true.
     * <p>
     * <p>
     * If you are creating your view programmatically, make sure you use {@link
     * FrameLayout#addView(View)} to add your view to the frame.
     *
    ...
    /**
     * Return the {@link StandOutLayoutParams} for the corresponding id. The system will set the
     * layout params on the view for this StandOut window. The layout params may be reused.
     *
     * @param id     The id of the window.
     * @param window The window corresponding to the id. Given as courtesy, so you may get the
     *               existing layout params.
     * @return The {@link StandOutLayoutParams} corresponding to the id. The layout params will be set
     * on the window. The layout params returned will be reused whenever possible, minimizing the
     * number of times getParams() will be called.
     */
    public abstract StandOutLayoutParams getParams(int id, Window window);

    /**
     * Implement this method to change modify the behavior and appearance of the window corresponding
     * to the id.
     * <p>
     * <p>
     * You may use any of the flags defined in {@link StandOutFlags}. This method will be called many
     * times, so keep it fast.
     * <p>
     * <p>
     * Use bitwise OR (|) to set flags, and bitwise XOR (^) to unset flags. To test if a flag is set,
     * use {@link Utils#isSet(int, int)}.
     *
     * @param id The id of the window.
    ...
    /**
     * Implement this method to set a custom title for the window corresponding to the id.
     *
     * @param id The id of the window.
    ...
    /**
     * Implement this method to set a custom icon for the window corresponding to the id.
     *
     * @param id The id of the window.
    ...
    /**
     * Return the title for the persistent notification. This is called every time {@link #show(int)}
     * is called.
     *
    ...
    /**
     * Return the message for the persistent notification. This is called every time {@link
     * #show(int)} is called.
     *
     * @param id The id of the window shown.
    ...
    /**
     * Return the intent for the persistent notification. This is called every time {@link #show(int)}
     * is called.
     * <p>
     * <p>
     * The returned intent will be packaged into a {@link PendingIntent} to be invoked when the user
    ...
    /**
     * Return the icon resource for every hidden window in this implementation. The icon will appear
     * in the default implementations of the hidden notifications.
     *
     * @return The icon.
     */
    ...
    /**
     * Return the title for the hidden notification corresponding to the window being hidden.
     *
    ...
    /**
     * Return the message for the hidden notification corresponding to the window being hidden.
     *
     * @param id The id of the hidden window.
    ...
    /**
     * Return the intent for the hidden notification corresponding to the window being hidden.
     * <p>
     * <p>
     * The returned intent will be packaged into a {@link PendingIntent} to be invoked when the user
     * clicks the notification.
    ...
    /**
     * Return a persistent {@link Notification} for the corresponding id. You must return a
     * notification for AT LEAST the first id to be requested. Once the persistent notification is
     * shown, further calls to {@link #getPersistentNotification(int)} may return null. This way
     * Android can start the StandOut window service in the foreground and will not kill the service
     * on low memory.
     * <p>
     * <p>
     * As a courtesy, the system will request a notification for every new id shown. Your
     * implementation is encouraged to include the {@link PendingIntent#FLAG_UPDATE_CURRENT} flag in
     * the notification so that there is only one system-wide persistent notification.
     * <p>
     * <p>
     * See the StandOutExample project for an implementation of {@link #getPersistentNotification(int)}
     * that keeps one system-wide persistent notification that creates a new window on every click.
     *
     * @param id The id of the window.
     * @return The {@link Notification} corresponding to the id, or null if you've previously returned
     * a notification.
     */
    public Notification getPersistentNotification(int id) {
        ...
        String channel_id = APPS_NOTIFICATION_CHANNEL_ID + id;
        NotificationChannelUtils.createNotificationChannel(c, channel_id);
    ...
    /**
     * Return a hidden {@link Notification} for the corresponding id. The system will request a
     * notification for every id that is hidden.
     * <p>
     * <p>
     * If null is returned, StandOut will assume you do not wish to support hiding this window, and
     * will {@link #close(int)} it for you.
     * <p>
     * <p>
     * See the StandOutExample project for an implementation of {@link #getHiddenNotification(int)}
     * that for every hidden window keeps a notification which restores that window upon user's
     * click.
     *
     * @param id The id of the window.
    ...
    public Notification getHiddenNotification(int id) {
        ...
        NotificationChannelUtils.createNotificationChannel(c, APPS_HIDDEN_NOTIFICATION_CHANNEL_ID);
        return new NotificationCompat.Builder(c, APPS_HIDDEN_NOTIFICATION_CHANNEL_ID)
                .setSmallIcon(icon)
                .setChannelId(APPS_HIDDEN_NOTIFICATION_CHANNEL_ID)
                .setContentTitle(contentTitle)
                .setContentText(contentText)
                .setTicker(tickerText)
                .setOngoing(true)
    ...
    /**
     * Return the animation to play when the window corresponding to the id is shown.
     *
     * @param id The id of the window.
    ...
    /**
     * Return the animation to play when the window corresponding to the id is hidden.
     *
     * @param id The id of the window.
     * @return The animation to play or null.
     */
    ...
    /**
     * Return the animation to play when the window corresponding to the id is closed.
     *
     * @param id The id of the window.
     * @return The animation to play or null.
     */
    ...
    /**
     * Implement this method to set a custom theme for all windows in this implementation.
     *
    ...
    /**
     * You probably want to leave this method alone and implement {@link #getDropDownItems(int)}
     * instead. Only implement this method if you want more control over the drop down menu.
     * <p>
     * <p>
     * Implement this method to set a custom drop down menu when the user clicks on the icon of the
     * window corresponding to the id. The icon is only shown when {@link
     * StandOutFlags#FLAG_DECORATION_SYSTEM} is set.
     *
     * @param id The id of the window.
     * @return The drop down menu to be anchored to the icon, or null to have no dropdown menu.
     */
    public PopupWindow getDropDown(final int id) {
        ...
        items.add(new DropDownListItem(
                R.drawable.ic_cancel_white_24dp, "退出"
        ...
        for (final DropDownListItem item : items) {
            ViewGroup listItem = (ViewGroup) mLayoutInflater.inflate(
                    R.layout.window_menu_listitem, null);
            ...
            description.setTextColor(getPrimaryTextColor());
    ...
    /**
     * Implement this method to populate the drop down menu when the user clicks on the icon of the
     * window corresponding to the id. The icon is only shown when {@link
     * StandOutFlags#FLAG_DECORATION_SYSTEM} is set.
     *
     * @param id The id of the window.
     * @return The list of items to show in the drop down menu, or null or empty to have no dropdown
     * menu.
     */
    ...
    /**
     * Implement this method to be alerted to touch events in the body of the window corresponding to
     * the id.
     * <p>
     * <p>
     * Note that even if you set {@link StandOutFlags#FLAG_DECORATION_SYSTEM}, you will not receive
     * touch events from the system window decorations.
     *
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id is about to be
     * shown. This callback will occur before the view is added to the window manager.
     *
     * @param id     The id of the view, provided as a courtesy.
     * @param window The view about to be shown.
     * @return Return true to cancel the view from being shown, or false to continue.
     * @see #show(int)
     */
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id is about to be
     * hidden. This callback will occur before the view is removed from the window manager and {@link
     * #getHiddenNotification(int)} is called.
     *
     * @param id     The id of the view, provided as a courtesy.
     * @param window The view about to be hidden.
     * @return Return true to cancel the view from being hidden, or false to continue.
     * @see #hide(int)
     */
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id is about to be
     * closed. This callback will occur before the view is removed from the window manager.
     *
     * @param id     The id of the view, provided as a courtesy.
     * @param window The view about to be closed.
     * @return Return true to cancel the view from being closed, or false to continue.
     * @see #close(int)
     */
    ...
    /**
     * Implement this callback to be alerted when all windows are about to be closed. This callback
     * will occur before any views are removed from the window manager.
     *
     * @return Return true to cancel the views from being closed, or false to continue.
     * @see #closeAll()
     */
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id has received some
     * data. The sender is described by fromCls and fromId if the sender wants a result. To send a
     * result, use {@link #sendData(int, Class, int, int, Bundle)}.
     *
     * @param id          The id of your receiving window.
     * @param requestCode The sending window provided this request code to declare what kind of data
     *                    is being sent.
     * @param data        A bundle of Parcelable data that was sent to your receiving window.
     * @param fromCls     The sending window's class. Provided if the sender wants a result.
     * @param fromId      The sending window's id. Provided if the sender wants a result.
     */
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id is about to be
     * updated in the layout. This callback will occur before the view is updated by the window
     * manager.
     *
     ...
     * @return Return true to cancel the window from being updated, or false to continue.
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id is about to be
     * bought to the front. This callback will occur before the window is brought to the front by the
     * window manager.
     *
     * @param id     The id of the window, provided as a courtesy.
     * @param window The window about to be brought to the front.
     * @return Return true to cancel the window from being brought to the front, or false to continue.
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id is about to have
     * its focus changed. This callback will occur before the window's focus is changed.
     *
     * @param id     The id of the window, provided as a courtesy.
     * @param window The window about to be brought to the front.
     * @param focus  Whether the window is gaining or losing focus.
     * @return Return true to cancel the window's focus from being changed, or false to continue.
     * @see #focus(int)
     */
    ...
    /**
     * Implement this callback to be alerted when a window corresponding to the id receives a key
     * event. This callback will occur before the window handles the event with {@link
     * Window#dispatchKeyEvent(KeyEvent)}.
     *
     * @param id     The id of the window, provided as a courtesy.
     * @param window The window about to receive the key event.
     * @param event  The key event.
     * @return Return true to cancel the window from handling the key event, or false to let the
     * window handle the key event.
    ...
    public final synchronized Window show(int id) {
        ...
        if (window.visibility == Window.VISIBILITY_VISIBLE) {
            Log.d(TAG, "Tried to show(" + id + ") a window that is already shown.");
            return null;
        }
    ...
    /**
     * Hide a window corresponding to the id. Show a notification for the hidden window.
     *
     * @param id The id of the window.
     */
     ...
     * result, provide the id of the sender.
     *
    ...
    public final void sendData(int fromId,
                               ...
                               Bundle data) {
        WindowAgreement.sendData(this, toCls, toId, requestCode, data,
                getClass(), fromId);
    }

    /**
     * Bring the window corresponding to this id in front of all other windows. The window may flicker
     * as it is removed and restored by the system.
     *
    ...
    /**
     * Request focus for the window corresponding to this id. A maximum of one window can have focus,
     * and that window will receive all key events, including Back and Menu.
     *
     * @param id The id of the window.
    ...
    /**
     * Remove focus for the window corresponding to this id. Once a window is unfocused, it will stop
    ...
    /**
     * Courtesy method for your implementation to use if you want to. Gets a unique id to assign to a
     * new window.
     *
    ...
    /**
     * Return whether the window corresponding to the id exists. This is useful for testing if the id
     * is being restored (return true) or shown for the first time (return false).
     *
     * @param id The id of the window.
     * @return True if the window corresponding to the id is either shown or hidden, or false if it
     * has never been shown or was previously closed.
     */
    ...
    /**
     * Return the window corresponding to the id, if it exists in cache. The window will not be
     * created with {@link #createAndAttachView(int, FrameLayout)}. This means the returned value will
     * be null if the window is not shown or hidden.
     *
     * @param id The id of the window.
    ...
    /**
     * Change the title of the window, if such a title exists. A title exists if {@link
     * StandOutFlags#FLAG_DECORATION_SYSTEM} is set, or if your own view contains a TextView with id
     * R.id.title.
     *
     * @param id   The id of the window.
    ...
    /**
     * Change the icon of the window, if such a icon exists. A icon exists if {@link
     * StandOutFlags#FLAG_DECORATION_SYSTEM} is set, or if your own view contains a TextView with id
     * R.id.window_icon.
     *
     * @param id          The id of the window.
    ...
    /**
     * Remove focus for the window, which could belong to another application. Since we don't allow
     * windows from different applications to directly interact with each other, except for {@link
     * WindowAgreement#sendData(Context, Class, int, int, Bundle, Class, int)}, this method is private.
     *
    ...
    public class StandOutLayoutParams extends WindowManager.LayoutParams {
        ...
        /**
         * Special value for y position that represents the bottom of the screen.
         */
        public static final int BOTTOM = Integer.MAX_VALUE;
        /**
         * Special value for x or y position that represents the center of the screen.
         */
        public static final int CENTER = Integer.MIN_VALUE;
        /**
         * Special value for x or y position which requests that the system determine the position.
         */
        ...
        public StandOutLayoutParams(int id) {
            super(200, 200, Utils.getWindowType(),
         ...
         * @param minWidth  The minimum width of the window.
         * @param minHeight The mininum height of the window.
         * @param threshold The touch distance threshold that distinguishes a tap from a drag.
         */
        public StandOutLayoutParams(int id, int w, int h, int xpos, int ypos,
